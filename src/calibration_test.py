for i in [4]:
    ...
    for cluster in cluster_indices:# every cluster is a set of indexes informing a cluster of points 
        if len(cluster)>len(tool_index):
            tool_index = cluster
    # select points and make it too0
    ...
    seg.set_MaxIterations(50000)# max iteration for ransac
    seg.set_optimize_coefficients("true")
    seg.set_method_type(6)

    inliers, coefficients = seg.segment()# apply segementer 
    ...
    # make image from extracted points 
